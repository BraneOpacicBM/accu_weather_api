...
class DetailedView extends Component {
  ...
  componentDidMount = () => {
    ...
    window.addEventListener("load", reloadToHome, false);
  };
  ...
  render() {
    if (this.state.cityInfo !== null) {
      ...
      return (
        <div className="DetailedView" onClick={this.reloadToHome}>
          <div className="SectionHolder">
            <div className="CenteredCard">
              <DisplayInfo
                info={cityInfo.cityData.city.name}
                name
                entityInfo="city"
              />
              <DisplayInfo
                info={cityInfo.cityData.city.country}
                country
                entityInfo="country"
              />
              <DisplayInfo info={population} entityInfo="population" />
              <div className="FiveDaysForecast">
                <h3 className="FiveDaysHeading">
                  Next five days average forecast:
                </h3>
                <div className="eachDayHolder">{fiveDayForecast}</div>
              </div>
            </div>
...
export default withRouter(connect(mapStateToProps)(DetailedView));
